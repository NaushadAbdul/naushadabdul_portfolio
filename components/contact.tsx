...
export function Contact({ settings }: ContactProps) {
  return (
    <section
      id="contact"
      data-reveal
      className="reveal-slow border-b-[3px] border-ink px-4 py-20 sm:px-6 md:py-28 [--float-distance:-6px] [--float-duration:13.5s]"
    >
      <div className="mx-auto w-full max-w-7xl">
        ...
        <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:gap-16">
          ...
          <div>
            ...
            <div className="float-drift brut-border mt-9 bg-brut-blue p-5 shadow-brut-sm">
              <p className="font-mono text-[0.65rem] font-bold uppercase tracking-[0.25em] text-paper/70">
                Prefer email
              </p>
              <a
                href={`mailto:${settings.email}`}
                className="float-drift mt-2 block border-b-[3px] border-paper/40 pb-1 font-display text-lg break-all text-paper transition-colors hover:border-paper md:text-2xl"
              >
                {settings.email}
              </a>
            </div>
            ...
            <div className="mt-9 flex flex-wrap gap-4">
              <Button href="/projects" variant="outline" float>
                See the full archive
              </Button>
              {settings.resume_url ? (
                <Button href={settings.resume_url} variant="accent" accent="lime" target="_blank" rel="noreferrer noopener" float>
                  Download Résumé
