...
const OptionWheel = ({
  ...
  const playTick = useCallback(() => {
    const { soundUrl: tickSoundUrl, soundVolume: tickSoundVolume } = cfgRef.current;
    if (!tickSoundUrl) return;
    const now = performance.now();
    if (now - lastTickRef.current < 70) return;
    lastTickRef.current = now;
    if (!audioRef.current || audioUrlRef.current !== tickSoundUrl) {
      audioRef.current = new Audio(tickSoundUrl);
      audioRef.current.preload = 'auto';
      audioUrlRef.current = tickSoundUrl;
    }
    const audio = audioRef.current;
    audio.volume = Math.min(Math.max(tickSoundVolume ?? 0.5, 0), 1);
    audio.currentTime = 0;
    audio.play()?.catch(() => {});
  }, []);
  ...
  const applyTarget = useCallback(
    (value, snap) => {
      const cfg = cfgRef.current;
      const count = cfg.count || items.length;
      let v = value;
      ...
      if (idx !== selectedRef.current) {
        ...
        const item = cfg.items ? cfg.items[idx] : items[idx];
        onChangeRef.current?.(idx, item);
        playTick();
      }
      startLoop();
    },
    [items, startLoop, playTick]
  );
  ...
  const handlePointerMove = useCallback(
    e => {
      ...
      const rowH = cfgRef.current.rowH || 48;
      if (dragMovedRef.current) applyTarget(drag.start - dy / rowH, false);
    },
    [applyTarget]
  );
  ...
  const handleItemClick = useCallback(
    index => {
      ...
      const count = cfgRef.current.count || items.length;
      const cur = targetRef.current;
      let d = index - (((cur % count) + count) % count);
      if (cfgRef.current.loop && count > 1) {
        if (d > count / 2) d -= count;
        else if (d < -count / 2) d += count;
      }
      applyTarget(cur + d, true);
    },
    [items.length, applyTarget]
  );
